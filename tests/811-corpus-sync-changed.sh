...
# github-actions[bot] and pushes to the bare remote. Then the
# lesson: every recipes/*/build is committed 100755 (mode stability),
# and a second sync is `unchanged` (idempotence).
set -eu
cd "$(dirname "$0")/.." || exit 2
...
head1=$(git -C "$TMPD/corpus.git" rev-parse refs/heads/main)
[ "$head0" != "$head1" ] || fail 'bare HEAD did not move on a changed day'
assert_eq "$(git -C "$TMPD/corpus.git" log -1 --format='%an <%ae>' refs/heads/main)" \
...
n=$(git -C "$TMPD/corpus" ls-files -s \
...
n=$(git -C "$TMPD/corpus.git" ls-tree -r refs/heads/main \
    | awk '$4 ~ /^recipes\/[^\/]*\/build$/ { print $1 }' | sort -u)
