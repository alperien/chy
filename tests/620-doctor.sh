#!/bin/sh
# doctor: audit every installed package. Findings on stdout, paths
# $CHY_ROOT-relative, then exactly `doctor: clean` (exit 0) or
# `doctor: <n> problem(s)` (exit 1), n counting finding lines.
# Read-only: creates no directories, absent ones read as empty. Check 1
# is unresolvable NEEDED sonames in store ELFs, check 2 broken owned
# farm links, check 3 manifest drift (missing / foreign).
set -eu
cd "$(dirname "$0")/.." || exit 2
...
assert_eq "$(cat "$OUT")" 'doctor: clean' 'bare root: clean line only'
...
want=$(printf 'doctor: brok: broken link usr/bin/brok-tool\ndoctor: 1 problem(s)')
...
want=$(printf 'doctor: miss: missing usr/bin/miss-tool\ndoctor: 1 problem(s)')
...
want=$(printf 'doctor: forn: foreign usr/bin/forn-tool\ndoctor: 1 problem(s)')
...
want=$(printf '%s\n%s\n%s\n%s' \
    'doctor: brok: broken link usr/bin/brok-tool' \
    'doctor: forn: foreign usr/bin/forn-tool' \
    'doctor: miss: missing usr/bin/miss-tool' \
    'doctor: 3 problem(s)')
...
want=$(printf 'doctor: needy: needs %s (package: helperpkg)\ndoctor: 1 problem(s)' "$fake_m")
